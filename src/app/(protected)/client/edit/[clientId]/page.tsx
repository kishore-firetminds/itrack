...
export default function EditClientPage() {
    ...
    const fetchClient = async () => {
      try {
        const res = await api.get(
          URLS.GET_CLIENT_BY_ID.replace('{id}', clientId)
        );
        ...
        if (data) {
          ...
          const normalized: Partial<ClientFormData> = {
            ...
            district_name:
              data.city_name ??
              data.district_name ??
              (typeof data.city === 'string' ? String(data.city) : ''),
            postal_code: data.postal_code
              ...
              : '',
            postal_name:
              data.postal_code ?? data.pincode ?? data.postal_name ?? '',
          ...
          try {
            // states for country
            if (data.country_id) {
              const sRes = await api.get(URLS.GET_STATES, {
                params: { country_id: data.country_id },
              });
              const statesRes = sRes.data?.data ?? [];
              console.debug(
                'GET_STATES response for country:',
                data.country_id,
                statesRes
              );
              setStates(statesRes);
            ...
            if (data.state_id) {
              const dRes = await api.get(URLS.GET_DISTRICTS, {
                params: { state_id: data.state_id },
              });
              const districtsFetched = dRes.data?.data ?? [];
              console.debug(
                'GET_DISTRICTS response for state:',
                data.state_id,
                districtsFetched
              );
              ...
              if (rawDistrictVal) {
                const match = districtsFetched.find((dd: District) => {
                  const name = String(
                    dd.district_name ?? dd.name ?? ''
                  ).toLowerCase();
                  return (
                    String(dd.district_id) === String(rawDistrictVal) ||
                    name === String(rawDistrictVal).toLowerCase()
                  );
                });
                if (match) {
                  ...
                  setField(
                    'district_name',
                    String(match.district_name ?? match.name ?? '')
                  );
                } else {
            ...
            if (districtParam) {
              const pRes = await api.get(URLS.GET_PINCODES, {
                params: { city_id: districtParam },
              });
              const list = pRes.data?.data ?? [];
              console.debug(
                'GET_PINCODES response for district/city:',
                districtParam,
                list
              );
              ...
              if (vendorPostal) {
                const match = list.find(
                  (p: Pincode) =>
                    String(p.pincode) === String(vendorPostal) ||
                    String((p as any).id) === String(vendorPostal)
                );
                if (match) {
                  setField(
                    'postal_code',
                    String(match.pincode ?? match.postal_code ?? '')
                  );
                  setField(
                    'postal_name',
                    String(match.postal_code ?? match.pincode ?? match.id ?? '')
                  );
                  ...
                  setPincodes((prev: Pincode[]) =>
                    prev.find(
                      (x: Pincode) =>
                        String(x.pincode) === String(match.pincode)
                    )
                      ? prev
                      : [match, ...prev]
                  );
                  ...
                  setPincodes((prev: Pincode[]) =>
                    prev.find(
                      (x: Pincode) => String(x.pincode) === String(vendorPostal)
                    )
                      ? prev
                      : [
                          {
                            pincode: String(vendorPostal),
    ...
    const fetchDistricts = async () => {
      try {
        ...
        if (formData.district_id) {
          const m = fetched.find(
            (d: District) =>
              String(d.district_id ?? d.id) === String(formData.district_id) ||
              String(d.district_name ?? d.name ?? '').toLowerCase() ===
                String(formData.district_id).toLowerCase()
          );
          if (m)
            setField('district_name', String(m.district_name ?? m.name ?? ''));
        }
        if (!isInitialLoad) {
          setField('district_id', '');
          setField('postal_code', '');
  ...
  useEffect(() => {
    ...
    const fetchPincodes = async () => {
      try {
        const res = await api.get(URLS.GET_PINCODES, {
          params: { city_id: formData.district_id },
        });
        ...
        if (formData.postal_code) {
          const exists = (filtered.length ? filtered : list).find(
            (p: Pincode) => String(p.pincode) === String(formData.postal_code)
          );
          if (!exists) {
            setPincodes((prev: Pincode[]) => [
              {
                pincode: String(formData.postal_code),
                lat: formData.lat ?? '',
                lng: formData.lng ?? '',
              },
              ...prev,
  ...
  const handlePostalSelect = (v: string) => {
    ...
    const m = pincodes.find(
      (p: Pincode) => String(p.pincode ?? p.postal_code ?? p.id) === String(v)
    );
    if (m) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (clientId) {
        const endpoint = (URLS.UPDATE_CLIENT as string).replace(
          '{id}',
          clientId
        );
      ...
      <Card className="p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <Label htmlFor="company_id" className="pb-3">
                  Company
                </Label>
                <Select
              ...
              <div>
                <Label htmlFor="business_typeId" className="pb-3">
                  Business Type
                </Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    {businessTypes.map((bt) => (
                      <SelectItem
                        key={String(bt.business_typeId)}
                        value={String(bt.business_typeId)}
                      >
          ...
          <div className="sm:col-span-2">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <Label htmlFor="address_1" className="pb-3">
                  Address
                </Label>
                <Input
                  id="address_1"
              ...
              <div>
                <Label htmlFor="country_id" className="pb-3">
                  Country
                </Label>
                <Select
              ...
              <div>
                <Label htmlFor="state_id" className="pb-3">
                  State
                </Label>
                <Select
              ...
              <div>
                <Label htmlFor="district_id" className="pb-3">
                  District / City
                </Label>
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    const m = districts.find(
                      (d) =>
                        String(d.district_id ?? d.id ?? d?.value) === String(v)
                    );
                    setField(
                      'district_name',
                      m ? String(m.district_name ?? m.name ?? '') : ''
                    );
                  }}
                ...
                >
                  <SelectTrigger className="w-full">
                    <SelectValue>
                      {formData.district_name || 'Select a district or city'}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {districts.map((d) => (
                      <SelectItem
                        key={String(d.district_id ?? d.id ?? d.value)}
                        value={String(d.district_id ?? d.id ?? d.value)}
                      >
              ...
              <div>
                <Label htmlFor="postal_code" className="pb-3">
                  Postal Code
                </Label>
                <Select
                ...
                >
                  <SelectTrigger className="w-full">
                    <SelectValue>
                      {formData.postal_name || 'Select a postal code'}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {pincodes.map((p: any) => (
                      <SelectItem
                        key={String(p.pincode ?? p.postal_code ?? p.id)}
                        value={String(p.pincode ?? p.postal_code ?? p.id)}
                      >
              ...
              <div>
                <Label htmlFor="lat" className="pb-3">
                  Latitude
                </Label>
                <Input
                  id="lat"
              ...
              <div>
                <Label htmlFor="lng" className="pb-3">
                  Longitude
                </Label>
                <Input
                  id="lng"
          ...
          <div className="sm:col-span-2">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 items-end">
              <div>
                <Label htmlFor="visiting_startTime" className="pb-3">
                  Start Time
                </Label>
                <Input
                  ...
                  onChange={(e) =>
                    setField('visiting_startTime', e.target.value)
                  }
                />
              </div>
              <div>
                <Label htmlFor="visiting_endTime" className="pb-3">
                  End Time
                </Label>
                <Input
                  id="visiting_endTime"
