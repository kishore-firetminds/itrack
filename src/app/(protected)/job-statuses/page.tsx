...
export default function JobStatusPage() {
  ...
  const handleFormSubmit = async (data: Record<string, any>) => {
    const payload = {
      job_status_title: data.job_status_title,
      status: data.status === 'active',
      job_status_color_code: data.job_status_color_code,
    };
    try {
      if (editData) {
        await api.put(
          `${URLS.GET_JOBSTATUSES}/${editData.job_status_id}`,
          payload
        );
      } else {
      ...
      console.error('Error saving job status', err);
    }
  };
                ...
                <TableRow key={w.job_status_id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  ...
                  <TableCell className="text-center">
                    <span
                      ...
                      style={{
                        backgroundColor: w.job_status_color_code,
                        color: '#fff',
                      }}
                    >
